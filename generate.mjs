 ...
 *   OUTPUT_PATH  - where to write the SVG (default: github-jet.svg)
 */
...
// Cyberpunk neon cyan-blue theme colors
const FLASH_COLOR = "#00f2fe"; 
const BULLET_COLOR = "#38bdf8"; 
const BLAST_COLOR = "#00f2fe"; 
...
async function fetchWeeks() {
  ...
  const ip = await new Promise((resolve) => {
    dns.lookup("api.github.com", { family: 4, all: false }, (err, address) => {
      resolve(err || !address ? process.env.GITHUB_API_IP || "20.207.73.85" : address);
    });
  });
  ...
  const res = await new Promise((resolve, reject) => {
    ...
    const req = https.request(requestOptions, (response) => {
      ...
      response.on("end", () => {
        resolve({ statusCode: response.statusCode || 0, body: data, ok: response.statusCode && response.statusCode >= 200 && response.statusCode < 300 });
      });
    });
  ...
  if (!res.ok) {
    throw new Error(`GitHub API error ${res.statusCode}: ${res.body}`);
  }
...
function buildJet() {
  ...
  <g transform="translate(0,0)">
    <polygon points="0,-16 8,6 4,3 -4,3 -8,6" fill="#00f2fe" stroke="#38bdf8" stroke-width="1"/>
    <polygon points="-8,6 -14,12 -4,7" fill="#3b82f6"/>
    <polygon points="8,6 14,12 4,7" fill="#3b82f6"/>
    <circle cx="0" cy="-6" r="2.2" fill="#c9e6ff"/>
    <polygon points="-3,7 3,7 0,15" fill="#00f2fe">
